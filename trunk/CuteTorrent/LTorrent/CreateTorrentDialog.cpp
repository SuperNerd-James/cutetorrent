...
{
	path = QFileDialog::getExistingDirectory(this, QString::fromLocal8Bit(tr("Открыть Папку").toAscii().data()),
                                             "",
...
void CreateTorrentDialog::BrowseFile()
{
	path =  QFileDialog::getOpenFileName(this,
		QString::fromLocal8Bit(tr("Открыть файл").toAscii().data()), "", QString::fromLocal8Bit("Any File (*.*)"));
	pathEdit->setText(path);
}
...
/*	info->describtion	= discribtionEdit->text().toStdString();
	...
	if (path.length()==0)
	{	QMessageBox::information(this, QString::fromLocal8Bit(tr("Создание торрентеа").toAscii().data()),
		QString::fromLocal8Bit(tr("Введите имя файла или папки").toAscii().data()));
		delete creator;
	...
	if (trackers.count()==0)
	{	if (QMessageBox::No == QMessageBox::information(this, QString::fromLocal8Bit(tr("Создание торрентеа").toAscii().data()),
		QString::fromLocal8Bit(tr("Вы не ввели ни одного торрент рекера.\nУверены что хотите продолжить?").toAscii().data()),
	...
	QString save_path = QFileDialog::getSaveFileName(this,
		QString::fromLocal8Bit(tr("Сохранение торрента").toAscii().data()),"",
...
void CreateTorrentDialog::ShowCreationSucces(QString filename)
{
	...
	{
		QMessageBox::information(this,QString::fromLocal8Bit(tr("Создание торрентеа").toAscii().data()),
			QString::fromLocal8Bit(tr("Создание торрента прошло успешно\nФайл сохранен %1").arg(filename).toAscii().data()));
	}
...
{
	QMessageBox::information(this, QString::fromLocal8Bit(tr("Создание торрентеа").toAscii().data()),
		QString::fromLocal8Bit(tr("Произошла ошибка при создании торрента:\n %1").arg(msg).toAscii().data()));
	progressBar->setValue(0);
	createButton->setEnabled(true);
	delete creator;
	creator = NULL;
