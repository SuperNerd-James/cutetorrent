...
void TorrentManager::initSession()
{
    ...
    QFile path_infohashFile(dataDir+"CT_DATA/path.resume");
    ...
    {
        MyMessageBox::warning(NULL,"Warning",tr("ERR_NO_FILE_PATH_RESUME_IF_FIRST_TIME_THEN_OK"));
    ...
    for (QStringList::iterator i=torrentFiles.begin();i!=torrentFiles.end();++i)
    {
        ...
        if (!ec)
            if (save_path_data.contains(to_hex(t->info_hash().to_string()).c_str()))
            {
                ...
                AddTorrent(dir.filePath(*i),torrentData.first,ec,QMap<QString,int>(),torrentData.second);
            }
...
void TorrentManager::handle_alert(alert* a)
{
    ...
    case storage_moved_alert::alert_type:
    {
        ...
        {
            QPair<QString,QString> torrentData = save_path_data[info_hash];
            save_path_data[info_hash]=qMakePair(QString::fromStdString(h.save_path()),torrentData.second);
        }
        else
        {
            QPair<QString,QString> torrentData=qMakePair(QString::fromStdString(h.save_path()),QString(""));
            save_path_data.insert(info_hash,torrentData);
        }
...
bool TorrentManager::AddTorrent(QString path, QString save_path,error_code& ec,QMap< QString , qint32 > filePriorities,QString group,bool sequntial)
...
void TorrentManager::HandleGroupChange(QString group, QString infohash)
{
    if (save_path_data.contains(infohash))
    {
        QString torrentData = save_path_data[infohash].first;
        save_path_data[infohash]=qMakePair(torrentData,group);
    }
    UpdatePathResumeAndLinks();
}
